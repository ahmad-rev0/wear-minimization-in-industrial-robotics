...
function defaultLayout(n: number): JointPosition2D[] {
...
export function JointEditor({
  ...
  const [joints, setJoints] = useState<JointPosition2D[]>(
    () => initialLayout ?? defaultLayout(Math.max(jointCount, 6))
  );
  const [ghostJoints] = useState<JointPosition2D[]>(() =>
    defaultLayout(Math.max(jointCount, 6))
  );
  ...
  const handleReset = () => {
    animateToLayout(defaultLayout(Math.max(jointCount, 6)));
